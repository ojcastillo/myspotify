...
def get_data_table(df):
    ...
    return [
        ...
        dbc.Alert(
            [
                "Spotify authentication required to create playlists. ",
                dash.html.A("Sign in with Spotify", href="/auth", className="alert-link"),
            ],
...
@dash.callback(
    dash.Output("library-container", "children"),
    dash.Input("selected-users-store", "data"),
)
def update_library(selected_users):
    if not selected_users:
        return []
    df = load_data(selected_users)
    return get_data_table(df)
...
def create_playlist(
    ...
    if not create_n_clicks:
        return success_alert_is_open, failure_alert_is_open, auth_alert_is_open

    # Check if Spotify auth is set up
    spotify_singleton = SpotifyClientSingleton()
    if not hasattr(spotify_singleton, "auth_manager"):
...
def layout():
    return dash.html.Div(id="library-container")
